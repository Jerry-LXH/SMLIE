function stats = collectEmitterStatistics(emitters,frames,ex_time,interval, ...
    ...
    %% --- Only use first 10 frames for brightness statistics ---
    nBrightFrames = min(10, frames);
    brightness_first10 = brightness_em(:,1:nBrightFrames);
    ...
    stats.sigma_em        = sigma_em;
    stats.sigma_loc_em    = sigma_loc_em;
    stats.bg_em           = bg_em;

    % brightness: first 10 frames only
    stats.brightness_mean = mean(brightness_first10,2,'omitnan') / ex_time;
    stats.brightness_sum  = sum(brightness_first10,2,'omitnan');
