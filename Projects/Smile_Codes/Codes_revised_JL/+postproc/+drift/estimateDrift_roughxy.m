function [corr_data, delta_sum] = estimateDrift_roughxy(raw_data, loc_total)
...
% [min_locs] sets the threshold of total localizations in ONE cluster to be able to estimate the drift. If either of the adjacant clusters lack enough locolizations then the drift correction in this round is skipped.
...
r = 3; % cloud-searching 
min_locs = 40; % the minium locoliaztions in one cluster for an effective drift correction.
...
for f=1:num_cluster:(floor(frames/10)-1)*num_cluster
    ...
    if size(Q,1) < min_locs || size(R,1) < min_locs
    % 点数太少，放弃本轮 drift 估计
        continue
    end
    ...
    dy_all = []; % collection of x-displacements
    dx_all = []; % collection of y-displacements
    ...
    for i = 1:numel(idx_cell)
        ...
        dxy = R(idxs,:) - Q(i,:);
        dy_all = [dy_all; dxy(:,1)];  % accumulate in rows
        dx_all = [dx_all; dxy(:,2)];
    end
    ...
    delta_trace(f_r,1) = delta_y / num_cluster; 
    delta_trace(f_r,2) = delta_x / num_cluster;
