...
%% Read Raw Data
    [raw_data, parameters.ex_time, gainDAC] = io.readSIFData(parameters.file_name); % 返回三维矩阵 [rows × cols × frames]，单位为绝对电子数 (e-)
...
%% Localization
    ...
    uncorrected_super_loc_total_raw = ...
        localize.MLE1.locMolecules(windowed_raw_data, uncorrected_detected_total, parameters.edge); % 输出 7 列: [row, col, brightness, background, sigma, sigma_loc, frame], sigma_loc 即 CRLB 估计的定位不确定度（像素）
...
%% Visualize All Emitter Trajectories
    figure;
    viz.plotImage(data, 1:parameters.frames, 'hot', ...
...
%% 仅用前半段数据拟合（t < t_cut）
    t_cut_options = [30, 40, 50, 60,100];  % 尝试不同截断点

    fprintf('t_cut (s) | τ (s)   | N(t_cut) | 拟合质量 R²\n');
    fprintf('──────────────────────────────────────────────\n');

    tau_results = zeros(size(t_cut_options));

    for i = 1:numel(t_cut_options)
        t_cut = t_cut_options(i);
        
        % 截取
        mask = t_axis <= t_cut;
        t_fit = t_axis(mask);
        N_fit = N_surviving(mask);
        
        % 拟合
        ft = fittype('a*exp(-x/tau)', 'independent', 'x', ...
            'coefficients', {'a','tau'});
        opts = fitoptions(ft);
        opts.StartPoint = [N_surviving(1), 30];
        opts.Lower = [0, 1];
        opts.Upper = [2*N_surviving(1), 200];
        
        f_cut = fit(t_fit(:), N_fit(:), ft, opts);
        tau_results(i) = f_cut.tau;
        
        % R²
        SS_res = sum((N_fit(:) - f_cut(t_fit(:))).^2);
        SS_tot = sum((N_fit(:) - mean(N_fit)).^2);
        R2 = 1 - SS_res/SS_tot;
        
        fprintf('  %3d     | %5.2f   |   %3d    |  %.4f\n', ...
            t_cut, f_cut.tau, N_fit(end), R2);
    end

    %可视化：不同截断点拟合对比
    figure('Color', 'w');
    colors = lines(numel(t_cut_options));

    subplot(1,2,1);
    stairs(t_axis, N_surviving, 'k-', 'LineWidth', 1.5); hold on;
    for i = 1:numel(t_cut_options)
        t_cut = t_cut_options(i);
        mask = t_axis <= t_cut;
        t_fit = t_axis(mask);
        ...
        opts = fitoptions(ft);
        opts.StartPoint = [N_surviving(1), 30];
        opts.Lower = [0, 1]; opts.Upper = [2*N_surviving(1), 200];
        f_cut = fit(t_fit(:), N_surviving(mask)', ft, opts);
        
        plot(t_axis, f_cut(t_axis), '-', 'Color', colors(i,:), ...
            'LineWidth', 1.5, 'DisplayName', ...
            sprintf('t_{cut}=%ds → \\tau=%.1fs', t_cut, f_cut.tau));
        xline(t_cut, '--', 'Color', colors(i,:), 'HandleVisibility','off');
    end
    xlabel('Time (s)'); ylabel('N surviving');
    legend('Location','northeast'); title('不同截断点拟合');
    set(gca, 'FontSize', 11);

    subplot(1,2,2);
    plot(t_cut_options, tau_results, 'ko-', 'LineWidth', 2, 'MarkerSize', 8);
    xlabel('t_{cut} (s)'); ylabel('\tau (s)');
    title('τ vs 截断点');
    yline(mean(tau_results), 'r--', sprintf('均值=%.1f s', mean(tau_results)));
    set(gca, 'FontSize', 11); grid on;

    sgtitle('Truncated Fit: 仅用早期数据估计 \tau');
...
%% Compute from States （已修复：仅计算非背景态的平均亮度，并新增总光子数估计）
