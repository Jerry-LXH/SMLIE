function [NPC_ShiftX,NPC_ShiftY,NPC_ShiftZ] = NP_Cloud_NoSort_3D_Pass2(ArRefXYZ, ArCmpXYZ, TolXYInPixel, TolZnm)  
%Call function to calculate the relative shift between two set of 3D-SMLM data through NP-Cloud-3D
%For Pass 2, calculation of the Z position of the cloud center also goes through iterative shifting in Z as done for XY. 
...
%Note: In the notation below, "localizations" are sometimes referred to as "molecules". We apologize for this inconsistency.
    ...
    for CurrentMoleInCmp=uint32(1):MoleNumCmp %Go through all the localizations in the comparing SMLM data to pair with the localizations the reference data
        ...
        if ~isempty(FoundY) %If localizaitons are found within the y range
            ...
            IndexMatchingX=find(abs(ArDistanceX) < TolXYInit & abs(ArDistanceZ) < TolZnmInit); %Find positions with X & Z ranges also within the initial search radius
    ...
    NPC_ShiftZ=mean(CurrentPassZSel(1:NumNearestMole))+Current_centerZ;
